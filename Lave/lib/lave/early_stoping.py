...
class NonSpikingPercentageStopping(EarlyStopping):
    def __init__(self, layer, threshold, proportion, old=False):
        ...
        self.count = 0  # Non spiking count
        self.scount = 0  # Spiking count

    def __call__(self, objective, network):
        self.total = len(objective.train_dataset)
        mask = (np.array(objective.img_spikes[self.layer]) < self.threshold).flatten()
        nsimg = np.count_nonzero(mask)
        self.count += nsimg
        self.scount += len(mask) - nsimg
        # print(
    ...
    def to_zeroinequality(self):
        # The higher the number of images that have spiked during the epoch
        # the lower the penalization
        if self.old:
            return self.count / self.total - self.proportion
        else:
            return 1 - self.scount / self.total - self.proportion
